...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleMessage(
    ...
    const message = `Новое сообщение в чате группы от ${payload.senderName}: ${payload.message}`;

    const chat = await this.messageService.getChatWithParticipants(
      payload.chatId,
    );

    for (const participant of chat.group.students) {
