		...
		<StyledMobileNavBottomWrapper>
			{!!groupId && (
				<Link
					...
					state={{ id: groupId }}
					className={`link ${locName === 'students' ? 'active' : ''}`}
				>
					<div className='content'>Моя группа</div>
				</Link>
			)}

			<Link
				to={pathsConfig.group_list}
				className={`link ${locName === 'groups' ? 'active' : ''}`}
			>
				<div className='content'>Группы</div>
			</Link>

			<Link
				to={pathsConfig.teachers_list}
				className={`link ${locName === 'teachers' ? 'active' : ''}`}
			>
				<div className='content'>Преподаватели</div>
			</Link>

			<Link
				to={pathsConfig.students_list}
				className={`link ${locName === 'students' ? 'active' : ''}`}
			>
				<div className='content'>Студенты</div>
			</Link>
		</StyledMobileNavBottomWrapper>
	)
}
